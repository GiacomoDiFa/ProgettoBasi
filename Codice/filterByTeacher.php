...
<body>
    <form action="" method="POST">
        <table>
            <tr>
                <td>
                    <h3>Select teacher of lessons</h3>
                </td>
                <td><div class="custom-select" style="width:200px;">
                    <select name="teacher">
                        ...
                        /* query execution */
                        $sql = "SELECT i.CodInsegnante,i.Nome,i.Cognome FROM insegnante as i;";
        ...
        <td><button>Confirm Teacher</button></td>    
        </tr>
        </table>
    </form>
    <h1>Schedule by Teacher</h1>
    <table>
        <?php
        error_reporting(E_ERROR);
        /* db connection */
        $lectionTeacher = $_POST["teacher"];
        $con = mysqli_connect('localhost', 'root', '', 'danza');
        if (mysqli_connect_errno()) {
            echo "Failed to connect to MySQL: " . mysqli_connect_error();
        }
        ...
        WHERE( (p.CodLezione = l.CodLezione) AND (p.CodInsegnante = i.CodInsegnante) AND (p.CodCoppia = c.CodCoppia) AND (p.CodSala = s.CodSala) AND (p.CodInsegnante = '$lectionTeacher'));";
        $result = mysqli_query($con, $sql);
        if (!$result) {
            die('Query error: ' . mysqli_error($con));
        }
