...
<body>
    <form action="" method="POST">
        <table>
            <tr>
                <td>
                    <h3>Select couple of lessons</h3>
                </td>
                <td><div class="custom-select" style="width:200px;">
                    <select name="couple">
                        ...
                        /* query execution */
                        $sql = "SELECT c.CodCoppia, c.NomeBallerino, c.CognomeBallerino,c.NomeBallerina,c.CognomeBallerina FROM coppia as c;";
        ...
        <td><button>Confirm Couple</button></td>    
        </tr>
        </table>
    </form>
    <h1>Schedule by Couple</h1>
    <table>
        <?php
        error_reporting(E_ERROR);
        /* db connection */
        $lectionCouple = $_POST["couple"];
        $con = mysqli_connect('localhost', 'root', '', 'danza');
        if (mysqli_connect_errno()) {
            echo "Failed to connect to MySQL: " . mysqli_connect_error();
        }
        ...
        WHERE( (p.CodLezione = l.CodLezione) AND (p.CodInsegnante = i.CodInsegnante) AND (p.CodCoppia = c.CodCoppia) AND (p.CodSala = s.CodSala) AND (p.CodCoppia = '$lectionCouple'));";
        $result = mysqli_query($con, $sql);
        if (!$result) {
            die('Query error: ' . mysqli_error($con));
        }
