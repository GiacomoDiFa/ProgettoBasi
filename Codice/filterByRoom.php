...
<body>
    <form action="" method="POST">
        ...
        <td><button>Confirm Room</button></td>    
        </tr>
        </table>
    </form>
    <h1>Schedule by Room</h1>
    <table>
        <?php
        error_reporting(E_ERROR);
        /* db connection */
        $lectionRoom = $_POST["room"];
        $con = mysqli_connect('localhost', 'root', '', 'danza');
        if (mysqli_connect_errno()) {
            echo "Failed to connect to MySQL: " . mysqli_connect_error();
        }
        ...
        WHERE( (p.CodLezione = l.CodLezione) AND (p.CodInsegnante = i.CodInsegnante) AND (p.CodCoppia = c.CodCoppia) AND (p.CodSala = s.CodSala) AND (p.CodSala = '$lectionRoom'));";
        $result = mysqli_query($con, $sql);
        if (!$result) {
            die('Query error: ' . mysqli_error($con));
        }
